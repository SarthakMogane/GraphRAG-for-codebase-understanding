...
class RepoScoutResult:
    ...
    api_calls_made: int
    scout_warnings: list[str] = field(default_factory=list)
...
class DeepScout:
    ...
    async def _quick_b3_score(
        ...
        try:
            ...
            latest_date_str = await self.gh.get_latest_commit_date(
                owner, repo, branch, self.installation_id
            )
            self._api_calls += 1
            if latest_date_str:
                # GitHub returns ISO 8601 strings like "2023-10-15T12:00:00Z"
                # Replace 'Z' with +00:00 for strict Python compatibility
                ts = datetime.fromisoformat(latest_date_str.replace("Z", "+00:00"))
                
                if ts > datetime.now(timezone.utc) - timedelta(days=365):
                    score += 1
        except Exception as e:
            logger.warning(f"Failed to parse commit date for {owner}/{repo}: {e}")
            pass
